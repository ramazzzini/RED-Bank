...
export class DonutChart extends ChildComponent{
    ...
    #createSvgPathElement(g) {
        ...
        const scale = 0.8
        const newSize = scale * this.size
        const radius = newSize / 2
        let accumulatedPercentage = 0

        this.data.forEach(slice => {
            const percentage = (slice.value / totalValue) * 100
            const [startX, startY] = this.#polarToCartesian(accumulatedPercentage, radius)
    ...
    /**
     * Generate an SVG element representing the donut chart.
